<div>
    ...
    @if($setting->type === 'textarea')
        <textarea 
            ...
            id="{{ $setting->key }}"
            rows="{{ in_array($setting->key, ['company_description', 'about_content']) ? '4' : '2' }}"
    ...
    @else
        <div class="flex gap-2">
            ...
            >
            @if(in_array($setting->key, ['company_logo', 'hero_image', 'about_image']))
            <input type="file" id="file_{{ $setting->key }}" accept="image/*" class="hidden" onchange="uploadImage(this, '{{ $setting->key }}')">
            <button type="button" onclick="document.getElementById('file_{{ $setting->key }}').click()" class="px-3 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 dark:from-emerald-500 dark:to-teal-500 text-white rounded-lg transition-all flex items-center gap-1.5 text-sm" title="Upload gambar">
