	...
	<div class="group relative bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl md:rounded-2xl shadow-md dark:shadow-xl hover:shadow-xl dark:hover:shadow-2xl border border-emerald-100 dark:border-emerald-800 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
		...
		<div class="relative">
			<div class="flex items-start justify-between mb-3 md:mb-4">
				...
				@if($unreadMessages > 0)
				<span class="px-2 md:px-3 py-1 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold rounded-full shadow-lg animate-pulse">{{ $unreadMessages }}</span>
				@endif
			</div>
			<div>
				<div class="text-xs md:text-sm font-semibold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-wide">Pesan Masuk</div>
				<div class="text-2xl md:text-4xl font-extrabold text-gray-900 dark:text-white mb-2">{{ $totalMessages }}</div>
	...
	<div class="group relative bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl md:rounded-2xl shadow-md dark:shadow-xl hover:shadow-xl dark:hover:shadow-2xl border border-blue-100 dark:border-blue-800 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
		...
		<div class="relative">
			...
			<div>
				...
				<div class="text-2xl md:text-4xl font-extrabold text-gray-900 dark:text-white mb-2">{{ $totalUnits }}</div>
	...
	<div class="group relative bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl md:rounded-2xl shadow-md dark:shadow-xl hover:shadow-xl dark:hover:shadow-2xl border border-amber-100 dark:border-amber-800 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
		...
		<div class="relative">
			...
			<div>
				...
				<div class="text-2xl md:text-4xl font-extrabold text-gray-900 dark:text-white mb-2">{{ $totalUsers }}</div>
	...
	<div class="group relative bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl md:rounded-2xl shadow-md dark:shadow-xl hover:shadow-xl dark:hover:shadow-2xl border border-purple-100 dark:border-purple-800 transition-all duration-300 hover:-translate-y-1 overflow-hidden">
		...
		<div class="relative">
			...
			<div>
				...
				<div class="text-2xl md:text-4xl font-extrabold text-gray-900 dark:text-white mb-2">{{ \App\Models\Facility::count() }}</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
	...
	<div class="bg-gradient-to-br from-emerald-500 to-emerald-600 dark:from-emerald-700 dark:to-emerald-800 p-4 md:p-6 rounded-xl md:rounded-2xl shadow-lg text-white">
		...
		<div class="space-y-2 md:space-y-3">
			<a href="{{ route('admin.units.index') }}" class="block bg-white/10 hover:bg-white/20 dark:bg-white/5 dark:hover:bg-white/10 backdrop-blur-sm rounded-lg md:rounded-xl p-3 md:p-4 transition-all group">
			...
			<a href="{{ route('admin.facilities.index') }}" class="block bg-white/10 hover:bg-white/20 dark:bg-white/5 dark:hover:bg-white/10 backdrop-blur-sm rounded-lg md:rounded-xl p-3 md:p-4 transition-all group">
				<div class="flex items-center gap-2 md:gap-3">
					<div class="p-1.5 md:p-2 bg-white/20 rounded-lg group-hover:scale-110 transition-transform">
						<svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
			...
			<a href="{{ route('admin.messages.index') }}" class="block bg-white/10 hover:bg-white/20 dark:bg-white/5 dark:hover:bg-white/10 backdrop-blur-sm rounded-lg md:rounded-xl p-3 md:p-4 transition-all group">
				<div class="flex items-center gap-2 md:gap-3">
					<div class="p-1.5 md:p-2 bg-white/20 rounded-lg group-hover:scale-110 transition-transform">
						<svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
					...
					<div class="flex-1">
						...
						<div class="text-xs text-white/70">{{ $unreadMessages }} pesan baru</div>
					</div>
					<svg class="w-4 h-4 md:w-5 md:h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
					</svg>
			...
			<a href="{{ route('admin.settings.index') }}" class="block bg-white/10 hover:bg-white/20 dark:bg-white/5 dark:hover:bg-white/10 backdrop-blur-sm rounded-lg md:rounded-xl p-3 md:p-4 transition-all group">
				<div class="flex items-center gap-2 md:gap-3">
					<div class="p-1.5 md:p-2 bg-white/20 rounded-lg group-hover:scale-110 transition-transform">
						<svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="mt-4 md:mt-6 bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl md:rounded-2xl shadow-md dark:shadow-xl border border-gray-100 dark:border-gray-700">
	<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 md:gap-4 mb-4 md:mb-6">
		...
		<a href="{{ route('admin.messages.index') }}" class="px-3 md:px-4 py-2 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-lg md:rounded-xl font-semibold text-xs md:text-sm transition-all flex items-center gap-2 w-fit">
			Lihat Semua
	...
	@if(isset($latestMessages) && $latestMessages->count())
		<div class="space-y-3">
						...
						<div class="flex-1 min-w-0">
							...
							<div class="mt-2 md:mt-3 flex flex-wrap items-center gap-2 md:gap-3">
								...
								<div class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
									...
									<span class="truncate max-w-[150px] sm:max-w-none">{{ $m->email }}</span>
								</div>
								@endif
								@if($m->phone)
								<div class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1">
...
@push('scripts')
<script id="dashboard-data" type="application/json">{!! json_encode(['months' => $months, 'counts' => $counts, 'unreadCounts' => $unreadCounts]) !!}</script>
@endpush
